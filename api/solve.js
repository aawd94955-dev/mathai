...
async function translateToEnglish(krText) {
  ...
  const { protectedText, mathBlocks } =
    extractAndProtectMath(cleanKr);

  try {
    const url =
      `https://translate.googleapis.com/translate_a/single?client=gtx&sl=ko&tl=en&dt=t&q=${encodeURIComponent(protectedText)}`;

    const response = await fetch(url);

    if (!response.ok) {
      throw new Error("번역 서버 응답 실패");
    }
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // ==========================================
    const wolframUrl =
      `https://api.wolframalpha.com/v2/query?appid=${WOLFRAM_APP_ID}&input=${encodeURIComponent(englishQuery)}&output=JSON&format=plaintext`;

    const wolframResponse =
      await fetch(wolframUrl);

    const wolframData =
      await wolframResponse.json();

    const pods =
      wolframData.queryresult?.pods;

    let wolframText =
      "WolframAlpha에서 유효한 계산 결과를 찾지 못했습니다.";

    if (pods && pods.length > 0) {

      wolframText = pods
        .filter(
          p => p.subpods?.[0]?.plaintext
        )
        .map(
          p =>
            `[${p.title}]\n${p.subpods
